...
class SIRIUS:
    def __init__(self):
        ...
        self.sd.add_servo_config(0, 180, self.sd.sec_to_ticks(600), self.sd.sec_to_ticks(2600))
        ...
        self.sd.turn_on()
        time.sleep_ms(5)
        self.sd.set_servo_to(num=3, degree=270 / 2)
        self.sd.set_servo_to(num=4, degree=0)
        ...
        self.menu_order = [
            ...
            [
                [
                    [["func ui_gripper_close"], "zumachen"],
                    [["func ui_gripper_rotate"], "Rotation"],
                ],
                "Greifer"
            ],
        ]
        ...
        self.angle_speed: float = 25 # °/s
        self.stepper_motor_speed = 1_500 # ns / step
        self.movement_type = "sinus" # quadrat, sinus, linear
        self.move_set = "move" # "set" = Fastest with no tick block

        self.gripper_placement = [0, 0] # first: 0 = 0% closed; second: relative rotate angle
    ...
    def set_warning_leds(self, intensity: int):
        # r, b, g | g and b are switched
        if intensity == 0:
            self.leds.set_rgb(0, 255, 120)
        elif intensity == 1:
            self.leds.set_rgb(0, 0, 255)
        elif intensity == 2:
            self.leds.set_rgb(255, 0, 120)
        elif intensity == 3:
            ...
            self.leds.set_freq(1, "g")
            self.leds.set_freq(1, "r")
    ...
    def slider(self, max_value: int, min_value: int, current_value: int, info_text: str = "", on_change = None):
        last_value: int = None
    ...
    def run_selected_function(self, func):
        ...
        elif func == "ui_gripper_close":
            self.ui_gripper_close()
            self.current_menu = [4, 0, -1]
            self.current_menu_str = self.menu_order[self.current_menu[0]][0][self.current_menu[1]][-1]

        elif func == "ui_gripper_rotate":
            self.ui_gripper_rotate()
            self.current_menu = [4, 1, -1]
            self.current_menu_str = self.menu_order[self.current_menu[0]][0][self.current_menu[1]][-1]
    ...
    def start_all_timer(self):
        ...
        self.emergency_button_timer.init(period=100, mode=machine.Timer.ONE_SHOT, callback=self.check_emergency_stop)
    ...
    def check_emergency_stop(self):
        ...
        if self.all_temps["max"] >= self.max_temp:
    ...
    def ui_gripper_close(self):

        def update_close(new_value):
            self.open_close_gripper(new_value / 100, self.gripper_placement[1])

        self.slider(max_value=100, min_value=0, current_value=self.gripper_placement[0] * 100, info_text="zumachen", on_change=update_close)

    def ui_gripper_rotate(self):

        def update_rotate(new_value):
            self.open_close_gripper(self.gripper_placement[0], new_value)
    ...
    def open_close_gripper(self, percentage, gripper_rotate):
        ...
        to_close = percentage * 70 # 70° == CLOSED
        self.sd.set_servo_to(num=4, degree=to_close)

        # gripper rotate
        gripper_rotate = min(max(gripper_rotate, -135), 135)
        self.sd.set_servo_to(num=3, degree=max(gripper_rotate - 7 + 135, 0) )# '-7' due to calibration and relative to absolute

        self.gripper_placement = [percentage, gripper_rotate]

    def auto_run_movement(self, start_delay: int = 500):
        self.set_warning_leds(2)
    ...
    def move_angles(self, _, change_leds: bool = True):
        ...
        # temp check if it took to long
        if time.ticks_ms() - total_start_time >= 2_000:
            self.read_and_analyse_temp(True)
        ...
        c1 = self.sd.servo_config[0]
        c2 = self.sd.servo_config[1]
        c3 = self.sd.servo_config[2]
        target_angles = self.to_angle

        sd_set_batch = self.sd.set_servos_batch

        if self.movement_type == "linear":
            mv_type = linear_percentage
        elif self.movement_type == "quadrat":
            mv_type = quadrat_percentage
        else:
            mv_type = sinus_percentage


        # timeing var
        start_time: float = time.ticks_ms()
        tick_start_time = time.ticks_ms()
        tick_time = 0

        while self.current_angles != target_angles:

            # get percentage
            percentage = mv_type(time.ticks_ms() - start_time)

            # calculate a new angle based on mode
            angle_1_angle = s1 + angle_1_distance * percentage
            angle_2_angle = s2 + angle_2_distance * percentage
            angle_3_angle = s3 + angle_3_distance * percentage
            ...
            tick_time = time.ticks_ms() - tick_start_time

            tick_start_time = time.ticks_ms()

            # time.sleep_ms(max(0, 5 - tick_time))


        self.Log(f"Movement finished in :{time.ticks_ms() - total_start_time} ms")

        if change_leds:
            self.set_warning_leds(1)
        # </editor-fold>
    ...
    def recalibrate_motors(self):
        ...
        # stop Motor 2
        self.to_angle = [0, [35, 80, 55]]
        self.move_set = "move"

        self.move_angles(True)
        self.open_close_gripper(1, 0)
        time.sleep_ms(500)

        self.sd.set_channel_duty(channel_num=0, duty_num=0)
        self.tmc.turn_off()

        self.set_warning_leds(0)
    ...
    def run_data_queue(self):
        ...
        if len(data_queue_copy) > 0:
            ...
            if data_to_process[0] == "gripper":
                self.open_close_gripper(data_to_process[1][0], data_to_process[1][1])
                time.sleep_ms(1)
        ...
        while True:
            ...
            if self.to_angle != self.current_angles:
                ...
                # linear "set" movement
                    ...
                    self.set_warning_leds(3)
                    self.move_one_tick(min(d_time / 1_000, 3 / 1_000))

                # not found
                else:
                    self.Log(f"No move_set called {self.move_set}. Reset to 'move'", 1)
                    self.move_set = "move"
            else:
                self.set_warning_leds(1)
                time.sleep_ms(2)
